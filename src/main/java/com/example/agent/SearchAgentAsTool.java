...
/**
 * Instead of passing GoogleSearchTool straight through to an agent as a tool, you can create
 * a dedicated search agent which encapsulate the search functionality and expands that agent
 * like a tool for a superior level agent.
 * <p>
 * This concept let you delegate complex behaviors (like search and summarize results) in a
 * specialized subagent. This approach is often useful for more complex workflows, as built-in
 * tools cannot be used with custom code-based tools.
 * <p>
 * For mundane questions, the agent will reply from his own knowledge base, but when asked
 * about recent events, it will delegate the search to the specialized search agent using the
 * GoogleSearchTool.
 *
 */
